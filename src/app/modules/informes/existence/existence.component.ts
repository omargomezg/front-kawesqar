...
} from "src/app/core/models/existence.model";
...
export class ExistenceComponent implements OnInit {
  ...
  ngOnInit() {
    ...
    this.sucursalService
      ...
      .subscribe(
        (data: SystemUser) => {
          data.relationSystemUserBranch.forEach(relation => {
            if (relation.isActive) {
              this.sucursal.push(relation.branch);
            }
            if (relation.isSelected) {
              this.request.idSucursal = relation.branch.id;
            }
          });
        },
        error => {
  ...
  getFamily() {
    this.serFamily.getFamilyByRut(this.localStorage.getRutUser()).subscribe(
      data => {
        this.family = data;
  ...
  getData() {
    this.servExistence.getExistence(this.request).subscribe(
      data => {
        this.existenceList = data;
        this.total = data.reduce(
          (sum, item) => sum + (item.cantidad + item.artValor),
          0
        );
      },
      error => {
